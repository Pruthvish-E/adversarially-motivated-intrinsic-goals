...
class MinigridNet(nn.Module):
    ...
    def forward(self, inputs, core_state=(), goal=torch.empty(0,0)):
...
def test(flags, num_episodes: int = 10):
    ...
    model = Net(gym_env.observation_space.shape, gym_env.action_space.n, state_embedding_dim=flags.state_embedding_dim, num_input_frames=flags.num_input_frames, use_lstm=flags.use_lstm, num_lstm_layers=flags.num_lstm_layers) 
    #model.share_memory()
   
    #model = GeneratorNet(gym_env.observation_space.shape, gym_env.action_space.n,flags.use_lstm,num_input_frames = 1000)
    #print("finally done see........................")
    #model = Generator(gym_env.observation_space.shape, gym_env.width, gym_env.height, num_input_frames=flags.num_input_frames)
    model.eval()
    checkpoint = torch.load(checkpointpath, map_location="cpu")
